...
function retrieveAllData(data, subject) {
    // var regionsAsHTML = "<table>";
    // var studyDataAsHTML = "<table>";
    // let regionsInTable = new Map();
    // var regionValues = [...regionsInTable.values()];
    // let studiesInTable = new Map();
    // var regionID = 0;
    // var singleRegionHTML;
    // var studyDataHTML;
    //
    ...
    //     if(data[i].subject == subject) {
    //         // Check if the region has already been added to regionsInTable
    //         if(regionValues.includes(data[i].region)) {
    //             // If it has, we do not need to add an element to regionsInTables
    //             // We only need to add it to studiesInTable
    //             studyDataHTML = createStudyData(data[i], regionsInTable., isFirst)
    //         }
    //         else {
    //             // Not in regionsInTable yet
    //             // Add it to regionsInTable
    //             singleRegionHTML = createRegionHeader(data[i].region, regionID);
    //             regionsInTable.set(regionID, singleRegionHTML);
    //             // and studiesInTable
    //             // Increment the regionID by 1
    //             regionsInTable.set(regionID, data[i].region);
    //             regionID += 1;
    //             regionValues = [...regionsInTable.values()]
    //             if(regionValues.includes(data[i].region)) {
    //                 console.log(data[i].region, " now exists in values");
    //             }
    //         }
    //     }
    // }
    ...
    // regionsInTable and studiesInTable will have the same key
    // regionsInTable Key

    // Join regionsAsHTML with studyDataAsHTML to form allHTMLData
    // return allHTMLData

    // studiesInTable.set(0, "testIterOne ");
    // var return_val = studiesInTable.get(0);
    // var joined_val = return_val + "testIterTwo";
    // studiesInTable.set(0, joined_val);
    // console.log(studiesInTable.get(0));
    var allData = new Map();
    var strippedData;
    var currentHTML;
    var joinedHTML
    for(let i = 0; i < data.length; i++) {
        if(data[i].subject = subject) {
            strippedData = data[i].region.replace(/\s/g, '');
            if(allData.get(strippedData)) {
                currentHTML = allData.get(strippedData);
                joinedHTML = currentHTML += getNewData(data[i]);
            }
            else {
                allData.set(strippedData, data[i].size);
                console.log(allData);
            }
        }
    }
...
// function createRegionHeader(region, regionID) {
//     return("<tr><td><a href='#" + regionID + "'>" + region + "</a></td></tr>");
// }
//
// function createStudyData(data, regionID, isFirst) {
//     if(isFirst) {
//         return ("<tr><td><a href='#" + regionID + "'>" + region + "</a></td></tr>");
//     }
// }
...
//                 console.log("Currently ", dictTableData);
//                 console.log("Changing to", data[i]);
